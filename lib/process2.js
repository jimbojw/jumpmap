      ...
      for (const searchCube of cube.searchCubes) {
        for (const target of searchCube.systems) {
          ...
          if (dx * dx + dy * dy + dz * dz < MAX_JUMP_DISTANCE_SQ) {
            if (!(system.name in links.stations)) {
              links.stations[system.name] = {from: system, to: {}, count: 0};
            }
            links.stations[system.name].to[target.name] = target;
            links.stations[system.name].count++;

            if (!(target.name in links.targets)) {
              links.targets[target.name] = {to: target, from: {}, count: 0};
            }
            links.targets[target.name].from[system.name] = system;
            links.targets[target.name].count++;
          }
        }
      }
    }
